  ...
  if (lotesTable) {
    lotesTable.addEventListener("click", function (e) {
      ...
      if (btnEditar) {
        ...
        console.log(
          "ID extraído para editar desde data-lote-id:",
          idLote,
          "tipo:",
          typeof idLote
        );
      ...
      if (btnEliminar) {
        ...
        const idLote = fila.getAttribute("data-lote-id");
        console.log(
          "ID extraído para eliminar desde data-lote-id:",
          idLote,
          "tipo:",
          typeof idLote
        );
...
function renderLotesTable(lotes) {
  ...
  if (!lotes || lotes.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="8" class="text-center text-muted">No hay lotes registrados</td></tr>';
    return;
  }
...
function editarLote(idLote) {
  ...
  fetch(`${API_URL}/${idLote}`)
    ...
    .then((lote) => {
      ...
      document.getElementById("loteFechaFabricacion").value =
        lote.fechaIngreso || "";
      document.getElementById("loteFechaVencimiento").value =
...
function filterLotes(searchTerm) {
  const filtered = lotesData.filter(
    (lote) =>
      (lote.numeroLote && lote.numeroLote.toLowerCase().includes(searchTerm)) ||
      (lote.producto &&
        lote.producto.nombre &&
        lote.producto.nombre.toLowerCase().includes(searchTerm))
  );
