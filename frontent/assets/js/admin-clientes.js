document.addEventListener('DOMContentLoaded', () => {
  ...
  const modalDetalles = new bootstrap.Modal(document.getElementById('clienteDetallesModal'));
  ...
  async function cargarClientes() {
    try {
      ...
      console.error(err);
      alert('No se pudo cargar los clientes. Revisa que el backend esté corriendo.');
    }
  }
  ...
  function mostrarClientes(list) {
    ...
    list.forEach(cli => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <button class="btn btn-info btn-sm me-1" onclick="verDetalles(${cli.idCliente})">
            <i class="bi bi-eye"></i> Ver
          </button>
          <button class="btn btn-warning btn-sm" onclick="editarCliente(${cli.idCliente})">
  ...
  window.verDetalles = function (id) {
    ...
    modalBody.innerHTML = `
      <ul class="list-group list-group-flush">
        ...
        <li class="list-group-item"><strong>Nombre:</strong> ${escapeHtml(cli.nombre)}</li>
        <li class="list-group-item"><strong>DNI:</strong> ${escapeHtml(cli.dni)}</li>
        <li class="list-group-item"><strong>Email:</strong> ${escapeHtml(cli.email || '-')}</li>
        <li class="list-group-item"><strong>Teléfono:</strong> ${escapeHtml(cli.telefono || '-')}</li>
        <li class="list-group-item"><strong>Dirección:</strong> ${escapeHtml(cli.direccion || '-')}</li>
        <li class="list-group-item"><strong>Fecha de nacimiento:</strong> ${cli.fechaNacimiento || '-'}</li>
        <li class="list-group-item"><strong>Condición crónica:</strong> ${cli.tieneCondicionCronica ? 'Sí' : 'No'}</li>
        <li class="list-group-item"><strong>Notas especiales:</strong> ${escapeHtml(cli.notasEspeciales || 'Ninguna')}</li>
        <li class="list-group-item"><strong>Acepta notificaciones:</strong> ${cli.aceptaNotificaciones ? 'Sí' : 'No'}</li>
      </ul>
    `;
    modalDetalles.show();
  };
  ...
  document.getElementById('editCondicionCronica').checked = cli.tieneCondicionCronica;
  document.getElementById('editAceptaNotificaciones').checked = cli.aceptaNotificaciones;
  modalEditar.show();
};
  ...
  formEditar.addEventListener('submit', async e => {
    ...
    tieneCondicionCronica: document.getElementById('editCondicionCronica').checked,
    aceptaNotificaciones: document.getElementById('editAceptaNotificaciones').checked 
    };
